...
Governance work must not select or imply a framework, database, API style, UI technology, deployment platform, or architecture. Do not add application code, scaffolds, package manifests, lockfiles, migrations, routes, components, containers, deployment files, ADRs, or product features unless a later product workstream expressly authorizes them.
...
Never commit secrets, credentials, tokens, private keys, production configuration, or sensitive local environment files.

Use synthetic data by default. Do not use real customer, guest, provider, booking, payment, notification, or communication data unless a later contract explicitly authorizes a controlled dataset and its handling requirements.
...
Record exact commands and results in the pull request. Confirm that the changed paths are authorized, the diff contains no accidental product implementation, and no secret or real production data is present.

Do not claim a GitHub Actions check passed until the check result is visible on the pull request or commit.

## 7. Open and maintain the pull request
...
## 9. Controller and owner authority
